...
function startup() {  
    ...
    d3.json(url).then(function(data){
        (data.names).forEach((name) => {menu.append("option").text(name).property("value", name)}); // populate drop down menu
...
function build_func(x) {           // build function
    d3.json(url).then((data) => { // pull data
        ...
        let trace2= {x: otu_ids1, y: sample_values1, text: otu_labels1, mode: "markers",marker: {size: sample_values1, color: otu_ids1, colorscale: "Earth"} }; // buble trace
